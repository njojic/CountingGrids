...
% 
%
% Written by Nebojsa Jojic and Alessandro Perina /
% {jojic, alperina}@microsoft.com

dbstop if error

if ~exist( 'options', 'var'); options = []; end
if ~isfield( options,'min_change'); options.min_change = 1e-11; end
...
if ~isfield( options,'plot_figure'); options.plot_figure = 1; end
if ~isfield( options,'normalize_data'); options.normalize_data = 0; end

if isfield( options, 'Lq'); monly=1; else monly=0; end
...
while ~converged
    ...
    if options.plot_figure == 1;
        figure(1), plot(1:iter, loglikelihood(1:iter),'.-r'); grid on; title('Log likelihood');  drawnow;
    end
...
%     if mod(iter,1)==0
%         opt.fig=200+iter/1;
%         [FO,WO] = show_text_in_grid(  pi, options.vocabulary, opt);
%     end
